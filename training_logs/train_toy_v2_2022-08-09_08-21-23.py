...
def init_model(num_classes):
    ...
    model = get_unet_model(norm_layer=norm_layer, num_classes=num_classes)
    model.apply(initializer.XavierGluon(rnd_type='gaussian', magnitude=1.0))

    return model, model_cfg


def train(model, model_cfg, args, epoch_len, num_points, num_epochs, start_epoch, train_proposals=False):
    loss_cfg = edict()
...
if __name__ == '__main__':
    ...
    num_classes = 5
    model, model_cfg = init_model(num_classes)
    ...
    # Start training
    print('#--------------Training with no proposals--------------#')
    train(model, model_cfg, args, epoch_len, num_points, num_epochs, 
          start_epoch=args.start_epoch, train_proposals=False)
    
    model.add_proposals_head()
    
    print('#---------------Training with proposals----------------#')
    train(model, model_cfg, args, epoch_len, num_points, num_epochs, start_epoch=args.start_epoch, train_proposals=True)
