...
class AdaptISTrainer(object):
    def __init__(self, args, model, model_cfg, loss_cfg,
        ...
        self.train_loader = DataLoader(trainset, batch_size=args.batch_size, pin_memory=True, shuffle=True, num_workers=args.workers, drop_last=True)
        
        self.val_loader = DataLoader(valset, batch_size=args.val_batch_size, pin_memory=True, shuffle=False, num_workers=args.workers, drop_last=True)
    ...
    def training(self, epoch):
        if self.summary_writer is None:
            self.summary_writer = log.SummaryWriterAvg(log_dir=str(self.args.logs_path), flush_secs=10, dump_period=self.tb_dump_period)
    ...
    def save_visualization(self, batch_data, outputs, global_step, prefix):
        ...
        if 'semantic' in batch_data:
            ...
            gt_mask = vis.visualize_mask(gt_segmentation.astype(np.int32), self.num_classes + 1)
            predicted_mask = vis.visualize_mask(predicted_label.astype(np.int32), self.num_classes + 1)
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            #print(batch_data['instances'].unique(), output.instances)
            ...
            #print(batch_data['semantic'].shape, output.semantic.shape)
    ...
    def _add_loss(self, loss_name, total_loss, losses_logging, validation, lambda_loss_inputs):
        ...
        if loss_weight > 0.0:
            ...
            #print(loss_name, loss_criterion)
